...
|---|---:|
| Văn bản công bố · Documents | **999** |
| Tổng số bản ghi đầu vào · Raw extract rows | 1,000 |
| Loại bỏ vì rỗng · Dropped (empty body) | 1 |
| Không có thân bài · `markdown` is null (source has no body) | 0 |
| Có cấu trúc · With structure layer | 900 |
| Có tệp đính kèm · With downloaded attachment | 500 |
| Trung vị ký tự · Median chars / doc | – |
| Trung vị trang · Median pages / doc | – |
| Trung vị đoạn văn · Median paragraphs / doc | – |
| Trung vị câu · Median sentences / doc | – |
...
structure). The 1024-D
`nvidia/custom-embed` embeddings, the global UMAP / PCA
coordinates, and the HDBSCAN cluster ids are computed during the
build (over 999 embeddable rows,
...
Mỗi điểm là một văn bản pháp luật; toạ độ là vector embedding 1024-D từ `nvidia/custom-embed` chiếu xuống 2D bằng **UMAP fit globally** trên toàn bộ kho văn bản (không chia batch) để toạ độ so sánh được giữa các tài liệu. Năm mặt phân hoạch: `scope`, `doc_type` (mã ngắn), `legal_type` (tên đầy đủ), `legal_area` (lĩnh vực pháp luật), `year`. Các nhãn tail-end (sau top-18) được dồn vào nhóm *Khác / Other* màu xám để chú giải đọc được. — Each dot is one legal document; coordinates are the 2D UMAP projection of a 1024-D embedding from `nvidia/custom-embed`, **fit globally across the whole corpus** so positions are directly comparable across documents. Five facets: `scope`, `doc_type` (canonical short code), `legal_type` (canonical full name), `legal_area` (subject domain), `year`. Tail-end labels beyond the top 18 are collapsed into a grey *Khác / Other* bucket to keep the legend legible.
...
4. **Embed** -- `nvidia/custom-embed` (1024-D) over the
...
5. **Reduce** -- PCA + UMAP on the embedding matrix + HDBSCAN
   ...
   **global in-process fit** over the full 999
   × 1024-D matrix so the projection is comparable
...
embeddable rows only (999-row corpus).
...
phép **MIT**; vui lòng kiểm tra điều khoản sử
...
**MIT**; please check the source-website terms of
...
@misc{vbpl_2026,
  ...
  author       = {TMQuan},
  year         = {2026},
  howpublished = {\url{https://huggingface.co/datasets/owner/name}},
