"""Reducer pipeline factory for vbpl: embeddings parquet -> reduced parquet.
...
The reducer fits all three projections (PCA / t-SNE / UMAP) in one
pass over the full ``embedding`` matrix so the output rows carry
globally-consistent 2- or 3-D coordinates plus an HDBSCAN
``cluster_id``. GPU-accelerated via cuML when ``cfg.reducer.prefer_gpu``
is set and cuML is importable; otherwise drops back to
sklearn / umap-learn / hdbscan.
"""
...
def build_reduce_pipeline(cfg: Any) -> Pipeline:
    """Return the vbpl Reducer :class:`Pipeline`."""
    return _build(
        cfg,
        site="vbpl",
