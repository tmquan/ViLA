...
from packages.pipeline.pdf_triage import PDF_TRIAGE_PIPELINES

#: Pipeline name -> factory. Keys double as ``--pipeline`` CLI choices.
#:
#: ``pdf_triage`` / ``pdf_native`` are the GPU-free native-extraction
#: path (see :mod:`packages.pipeline.pdf_triage`). They are an
#: alternative to ``parse``, not a replacement, so they are registered
#: for explicit selection but deliberately left out of
#: :data:`ALL_PIPELINES_ORDER`.
