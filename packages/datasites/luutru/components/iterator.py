"""luutru DocumentIterator: turn one downloaded file into one record.

Subclasses :class:`nemo_curator.stages.text.download.base.DocumentIterator`.
Given the path to a PDF/DOCX/DOC written by
:class:`LuutruDocumentDownloader`, emits exactly one dict record
carrying:

* ``doc_name``    - stable GUID (filename stem).
...
class LuutruDocumentIterator(DocumentIterator):
    """One record per luutru document."""
...
__all__ = ["LuutruDocumentIterator"]
