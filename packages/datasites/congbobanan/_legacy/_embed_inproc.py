"""In-process driver for the congbobanan Embedder stage.

The congbobanan counterpart to
:mod:`packages.datasites.anle._embed_inproc` (and a sibling to
congbobanan's own :mod:`_reduce_inproc`). Reads every extracted JSONL
under ``jsonl/``, embeds each judgment with the configured backend
(:class:`NimEmbedderStage` -- NIM or local HuggingFace runtime), and
writes per-doc parquets under ``parquet/embeddings/``. Output schema
matches ``--pipeline embed``; only the runtime differs (no Ray).

Use this on single-host boxes where cosmos-xenna cannot see the GPU
(e.g. a GB10 without ``pynvml``) and the Ray/xenna scheduler refuses the
``gpus=1.0`` embed actor. Resumable: docs already embedded are skipped.
...
def run(cfg, *, batch_size: int = 32) -> int:
    ...
    for start in range(0, len(df), batch_size):
        ...
        out = stage.process(
            DocumentBatch(task_id=f"embed_{start}", dataset_name="congbobanan", data=sub)
        ).to_pandas()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="In-process congbobanan Embedder (no Ray).")
    parser.add_argument("--config", type=Path, default=None)
    parser.add_argument("--output", type=Path, default=None)
    parser.add_argument("--batch-size", type=int, default=32)
    args = parser.parse_args(argv)

    config_path = args.config or find_site_config("congbobanan")
    overrides: list[str] = []
    if args.output:
