"""Upload the materialised congbobanan HF folder to a HuggingFace dataset repo.

Reads the parquet shards + dataset card produced by
:mod:`packages.datasites.congbobanan.hf_export` and pushes them to the
HuggingFace Hub.
...
Examples::
    ...
    # Default repo: tmquan/congbobanan-toaan-gov-vn (public)
    python -m packages.datasites.congbobanan.push_to_hf

    # Override repo + use a private repo
    python -m packages.datasites.congbobanan.push_to_hf \\
        --repo-id myorg/congbobanan --private

    # Dry-run: validate the folder + print what would happen
    python -m packages.datasites.congbobanan.push_to_hf --dry-run
...
DEFAULT_HF_DIR  = Path("data/congbobanan.toaan.gov.vn/hf")
DEFAULT_REPO_ID = "tmquan/congbobanan-toaan-gov-vn"
...
#: produces a wildly different shard count. At the full 1.37 M-doc
#: scale the defaults fan into ~138 ``documents`` / ``embed`` /
#: ``reduce`` shards and ~580 ``sentences`` shards, so a minimum of 1
#: catches any half-written run that left zero parquets on disk.
...
def _validate_shards(folder: Path) -> None:
    ...
    if len(documents) < MIN_DOCUMENTS_SHARDS:
        raise FileNotFoundError(
            ...
            f"`python -m packages.datasites.congbobanan.hf_export` before pushing."
        )
    ...
    for name, shards, minimum in (
        ...
        if shards and len(shards) < minimum:
            raise FileNotFoundError(
                ...
                f"Re-run `python -m packages.datasites.congbobanan.hf_export` "
                f"before pushing.",
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    return run_push_cli(
        ...
        description="Push the materialised congbobanan HF folder to HuggingFace.",
        default_commit_message=(
            "Vietnamese bản án corpus with sentence-level structure + "
