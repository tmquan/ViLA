...
composite stage::
    ...
    -> DocumentDownloadStage(CongbobananPDFDownloader)   # detail -> pages/, body -> files/
    -> DocumentIterateExtractStage(CongbobananIterator, CongbobananExtractor)

``CongbobananDownloadExtractStage`` is a thin
...
@dataclass
class CongbobananDownloadExtractStage(DocumentDownloadExtractStage):
    ...
    url_generator: CongbobananURLGenerator = field(default=None)  # type: ignore[assignment]
    downloader: CongbobananPDFDownloader = field(default=None)  # type: ignore[assignment]
    iterator: CongbobananIterator = field(default=None)  # type: ignore[assignment]
    extractor: CongbobananExtractor = field(default=None)  # type: ignore[assignment]
    ...
    def build(
        ...
        *,
        ...
        return cls(
            url_generator=CongbobananURLGenerator(
                start_id=start_id, end_id=end_id, detail_template=detail_template
            ),
            downloader=CongbobananPDFDownloader(
                str(files_dir),
            ...
            iterator=CongbobananIterator(
                pages_dir=str(pages_dir), detail_template=detail_template
            ),
            extractor=CongbobananExtractor(),
            url_limit=url_limit,
...
def _load_urls(args: argparse.Namespace) -> list[str]:
    ...
    gen = CongbobananURLGenerator(start_id=args.start, end_id=args.end)
...
def main(argv: list[str] | None = None) -> int:
    ...
    iterator = CongbobananIterator(pages_dir=str(pages_dir))
    extractor = CongbobananExtractor()
...
__all__ = ["CongbobananDownloadExtractStage", "main"]
